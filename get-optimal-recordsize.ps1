param (
	[Parameter(Mandatory)][ValidateRange(0,3)][int]$RaidLevel, # specify 0 for striped or mirrored pools
...
for($i=$MinPower;$i -le $MaxPower;++$i){
	...
	foreach($Item in $FileList){
		...
		$Parity=[Math]::Ceiling($SectorsPerBlock/($StripeWidth-$RaidLevel))*$RaidLevel
		...
		$Theoretical=$SectorsPerBlock*$StripeWidth/($StripeWidth-$RaidLevel)
